...
export const VALIDATION_RULES = {
  ...
  declaration: {
    ...
    activityType: { required: true, allowedValues: ['tarea', 'proyecto'] as const },
...
export function validateCourseFields(data: CourseFieldsInput): ValidationResult {
  const errors: ValidationError[] = [];
  ...
  if (!data.teacherEmail || (typeof data.teacherEmail === 'string' && data.teacherEmail.trim() === '')) {
    ...
    if (!emailResult.valid) {
      errors.push(
        ...emailResult.errors.map((e) => ({ ...e, field: 'teacherEmail' }))
      );
    }
  }
...
export function validateDeclarationFields(data: DeclarationFieldsInput): ValidationResult {
  const errors: ValidationError[] = [];
  ...
  } else if (!rules.activityType.allowedValues.includes(data.activityType as 'tarea' | 'proyecto')) {
    errors.push({
      field: 'activityType',
      rule: 'enum',
